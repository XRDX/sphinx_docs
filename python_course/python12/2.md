...
from com.nina.hello import Hello
...
上面也提到过，`type()` 函数可以查看一个类型或变量的类型，`Hello` 是一个 `class` ，它的类型就是 `type` ，而 `h` 是一个实例，它的类型就是 `com.nina.hello.Hello`。前面的 `com.nina` 是我的包名，`hello` 模块在该包名下。
